...
(mu/defn sort-drill :- [:maybe ::lib.schema.drill-thru/drill-thru.sort]
  ...
  (when (and column
    ...
    (when (or (not (lib.drill-thru.common/mbql-stage? query stage-number))
              (orderable-column? query stage-number column-ref))
...
(mu/defmethod lib.drill-thru.common/drill-thru-method :drill-thru/sort
  ...
  ([query                        :- ::lib.schema/query
   ...
   (if (lib.drill-thru.common/mbql-stage? query stage-number)
     (-> query
     ...
     (let [raw-query (lib.native/raw-native-query query)
           new-raw-query (str "SELECT * FROM ("
                              raw-query
                              ") ORDER BY "
                              (:name column)
                              " "
                              (str/upper-case direction))]
       (lib.native/with-native-query query new-raw-query)))))
